...
def create_stratified_split(X, y, test_size=0.2, val_size=0.1):
...
def load_sequences(file_path, sequence_length, step):
    ...
    # Extract features and labels for all rows
    features_list = []
    label_list = []
    ...
    print(f"\nCreating sequences with length {sequence_length} and step {step}:")
...
if __name__ == "__main__":
    ...
    csv_path = "C:\\Users\\memoo\\esp\\Project\\Fine_codes_1\\Training.csv"  # <-- Update with your CSV file path

    print("Loading and preprocessing data from single CSV...")
    print(f"Configuration: SEQ_LEN={SEQUENCE_LENGTH}, OVERLAP={OVERLAP}, BATCH_SIZE={BATCH_SIZE}")

    # Load sequences with improved parameters
    X_seq, y_seq = load_sequences(csv_path, SEQUENCE_LENGTH, SEQUENCE_LENGTH - OVERLAP)
    ...
    if len(y_seq) < 500:
        print("⚠️  WARNING: Very few sequences generated!")
    ...
    torch.save(lstm_model.state_dict(), "final_lstm_model.pt")
