...
df = df[['aoa', 'master_rssi', 'worker_rssi', 'IQ_master', 'IQ_worker', 'distance_m', 'angle_deg']]
...
n_class = len(selected_degrees) * len(selected_dists)
n_iter = 5
n_epochs = 100
n_batch = 32
...
handle_data = 0 # 0: keep all original data (data amount biases will affect the model), 
                ...
                # 2: pad data from all locations to have the n_data = maximum

# 0: aoa, 1: master_rssi, 2: worker_rssi, 3: IQ_master, 4: IQ_worker, 5: true_distance, 6: true_angle, 7: random
combs = [[4]]#[[0], [1], [3], [1,3], [2], [2,4], [1,2], [3,4], [1,2,3,4], [0,1,2,3,4]]
for comb in combs:
    used_features = comb
